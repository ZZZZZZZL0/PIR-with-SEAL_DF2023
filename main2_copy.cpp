...
int main(int argc, char *argv[]) {
    ...
    uint64_t number_of_items = 1 << 27;
    uint32_t N = 2048;
    size_t size_per_item = 20;

    size_t number_of_layer = 3;
    size_t item_of_commit = 1 << 9;
        ...
        for (size_t j = 0; j < commit_of_this_layer; j ++){
            if (i != number_of_layer - 1){
                for (size_t k = 0; k < item_of_commit; k ++){
                    ...
                    mpz_mul(sum, sum, a[item_of_commit - k]);
                    mpz_mod(sum, sum, order);
                    element_pp_pow(proof[i][j * item_of_commit + k], sum ,g1_pp);
                }
            }
            else{
                cout << "final layer\n";
                for (size_t k = 0; k < item_of_final_commit; k ++){
                    ...
                    mpz_mul(sum, sum, a[item_of_final_commit - k]);
                    mpz_mod(sum, sum, order);
                    element_pp_pow(proof[i][j * item_of_final_commit + k], sum ,g1_pp);
                }
            }
        }
    }
